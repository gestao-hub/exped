...
#  2) agente vivo e destravado (start se parou; restart se agent.log >15min)
#  3) BACKFILL: repesca pedido que ficou elegivel (sit 2/5/7) DEPOIS do cursor do agente passar
...
try {
  ...
  if (-not $proc) {
    if ($exe) { Start-Process $exe -WorkingDirectory (Split-Path $exe); Tick 'agente parado -> reiniciado' } else { Tick 'agente: exe nao achado' }
  } else {
...
try {
  ...
  $hwm    = [int]([regex]'"Hwm"\s*:\s*(\d+)').Match($stRaw).Groups[1].Value
